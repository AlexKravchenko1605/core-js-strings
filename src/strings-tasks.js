...
function sumOfCodes(str) {
  if (str) {
    ...
    for (let i = 0; i < arrayFromStr.length; i += 1) {
      sumOfCode += arrayFromStr[i].charCodeAt(0);
    }
    return sumOfCode;
  }
  return 0;
...
function findLongestWord(sentence) {
  const arrayFromSentence = sentence
    .split(' ')
    .sort((a, b) => b.length - a.length);
  return arrayFromSentence[0];
}
...
function extractNameFromTemplate(value) {
  const str = value.replace(/Hello, /gi, '');
...
function encodeToRot13(str) {
  ...
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (uncode.indexOf(str[i]) !== -1) {
      result += encode[uncode.indexOf(str[i])];
    } else {
      result += str[i];
    }
  }
